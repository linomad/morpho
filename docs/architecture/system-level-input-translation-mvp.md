...
- 输入兼容策略：
  - 主通道：AX 直读直写（零剪贴板污染）
  - 兜底通道：受控复制 + CGEvent 直接文本注入（选中优先；无选中时允许受控全文路径 `⌘A/⌘C`）
- 失败反馈：系统通知 + 菜单栏状态
- 兼容边界：优先支持标准 AX 可读写输入控件；AX 失败时支持“选中复制 + 粘贴写回”与“受控全文复制 + 粘贴写回”兜底
...
- 输入策略采用 AX First：优先 AX 直读直写，保证零剪贴板污染路径
- 当 AX 在现代 Electron 场景中解析失败时，尝试一次 `AXManualAccessibility` 激活并重试解析
- 仅在 AX 路径最终失败时启用受控复制兜底；优先处理选中，若无选中则执行受控全文路径（`⌘A/⌘C`）
- 兜底写回采用 CGEvent 直接文本注入，完全不操作剪贴板（零污染）
- 受控复制采用 deterministic copy handshake（probe token + changeCount 轮询 + 还原快照），降低旧剪贴板值误判
- AX 解析支持向父节点回溯与多文本类型解码（String/AttributedString/URL）

## Known Limits

- 非标准输入控件（尤其自绘组件）可能不暴露可写 AX 属性
- 对“AX 与受控复制均不可读”的输入控件，提示“当前输入控件不支持直接翻译”
- 安全输入框（密码类）不处理
...
2. 无选中行为：默认翻译并替换全文（必要时通过受控全文路径完成）。
