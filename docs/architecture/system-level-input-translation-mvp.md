...
- 输入兼容策略：
  - 主通道：AX 直读直写
  - 兜底通道：受控复制/粘贴（选中优先；无选中时允许受控全文路径 `⌘A/⌘C/⌘V`）
- 失败反馈：系统通知 + 菜单栏状态
...
- 仅在 AX 路径最终失败时启用受控粘贴兜底；优先处理选中，若无选中则执行受控全文路径（`⌘A/⌘C/⌘V`）
- 受控粘贴采用 deterministic copy handshake（probe token + changeCount 轮询 + 还原快照），降低旧剪贴板值误判
